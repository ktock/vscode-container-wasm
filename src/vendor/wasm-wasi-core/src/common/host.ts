...
import {
    ...
    random_get, sock_accept, sock_shutdown, sock_recv, sock_send, thread_spawn, thread_exit, roflags
} from './wasi';
...
export namespace WasiHost {
	export function create(connection: HostConnection): WasiHost {
		...
		const wasi: WasiHost = {
			...
			sock_recv: (fd: fd, ri_data_ptr: ptr<iovec>, ri_data_len: u32, ri_flags: riflags, ro_datalen_ptr: ptr<u32>, roflags_ptr: ptr<roflags>): errno => {
				try {
				    return connection.call(sock_recv, [fd, ri_data_ptr, ri_data_len, ri_flags, ro_datalen_ptr, roflags_ptr], memory(), sock_recv.transfers(memoryView(), ri_data_ptr, ri_data_len));
				} catch (error) {
					return handleError(error, Errno.inval);
				}
			},
			sock_send: (_fd: fd, _si_data_ptr: ptr<ciovec>, _si_data_len: u32, _si_flags: siflags, _si_datalen_ptr: ptr<u32>): errno => {
				try {
					return connection.call(sock_send, [_fd, _si_data_ptr, _si_data_len, _si_flags, _si_datalen_ptr], memory(), sock_send.transfers(memoryView(), _si_data_ptr, _si_data_len));
				} catch (error) {
					return handleError(error, Errno.inval);
				}
			},
