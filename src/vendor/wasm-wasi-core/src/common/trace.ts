...
import { Advise, Clockid, Errno, Fdflags, Fdstat, Filestat, Filetype, Fstflags, Lookupflags, Oflags, Prestat, Sdflags, Whence, advise, ciovec, clockid, dircookie, dirent, errno, event, exitcode, fd, fdflags, fdstat, filedelta, filesize, filestat, fstflags, iovec, lookupflags, oflags, prestat, rights, sdflags, subscription, timestamp, whence, riflags, roflags, siflags } from './wasi';
...
export namespace TraceMessage {
	export function create(): TraceMessage {
		...
		return {
			...
			sock_recv: (_memory: ArrayBuffer, result: errno, fd: fd, _ri_data_ptr: ptr<iovec>, _ri_data_len: u32, _ri_flags: riflags, _ro_datalen_ptr: ptr<u32>, _roflags_ptr: ptr<roflags>): string => {
				return `sock_recv(fd: ${fd}) => [result: ${Errno.toString(result)}]`;
			},
			sock_send: (_memory: ArrayBuffer, result: errno, fd: fd, _si_data_ptr: ptr<ciovec>, _si_data_len: u32, _si_flags: siflags, _si_datalen_ptr: ptr<u32>): string => {
				return `sock_send(fd: ${fd}) => [result: ${Errno.toString(result)}]`;
			},
