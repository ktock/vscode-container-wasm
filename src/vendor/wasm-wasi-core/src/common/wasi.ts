...
export namespace Rights {
	...
	export const sock_recv = 1n << 30n;
	export const sock_send = 1n << 31n;
	...
	export const All = fd_datasync | fd_read | fd_seek | fd_fdstat_set_flags |
		...
		path_unlink_file | poll_fd_readwrite | sock_shutdown | sock_accept | sock_recv | sock_send;
	...
	export function toString(value: rights): string {
		...
		if (contains(value, Rights.sock_shutdown)) { parts.push('sock_shutdown'); }
	        ...
	        if (contains(value, Rights.sock_recv)) { parts.push('sock_recv'); }
		if (contains(value, Rights.sock_send)) { parts.push('sock_send'); }
...
export namespace Roflags {
	...
	export const $transfer = U16.$transfer;
}
...
export namespace sock_recv {
	export const name: string = 'sock_recv';
        export const signature = WasiFunctionSignature.create([Fd.$param, Iovec.$ptr, U32.$param, Riflags.$param, U32.$ptr, Roflags.$ptr]);
	export function transfers(memory: DataView, ri_data_ptr: ptr<iovec>, ri_data_len: u32): ArgumentsTransfer {
	    return ArgumentsTransfer.create([Iovec.createTransfer(memory, ri_data_ptr, ri_data_len), U32.$transfer, Roflags.$transfer]);
	}
	export type ServiceSignature = (memory: ArrayBuffer, fd: fd, ri_data_ptr: ptr<iovec>, ri_data_len: u32, ri_flags: riflags, ro_datalen_ptr: ptr<u32>, roflags_ptr: ptr<roflags>) => Promise<errno>;
	WasiFunctions.add(sock_recv);
}
...
export namespace sock_send {
	export const name: string = 'sock_send';
        export const signature = WasiFunctionSignature.create([Fd.$param, Ciovec.$ptr, U32.$param, Siflags.$param, U32.$ptr]);
	export function transfers(memory: DataView, si_data_ptr: ptr<ciovec>, si_data_len: u32): ArgumentsTransfer {
		return ArgumentsTransfer.create([Ciovec.createTransfer(memory, si_data_ptr, si_data_len), U32.$transfer]);
	}
	export type ServiceSignature = (memory: ArrayBuffer, fd: fd, si_data_ptr: ptr<ciovec>, si_data_len: u32, si_flags: siflags, si_datalen_ptr: ptr<u32>) => Promise<errno>;
	WasiFunctions.add(sock_send);
}
