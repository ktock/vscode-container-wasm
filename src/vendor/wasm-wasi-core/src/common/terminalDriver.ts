...
import { fd, fdflags, fdstat, filestat, Filetype, Rights, rights, filesize } from './wasi';
...
export function create(deviceId: DeviceId, terminal: WasmPseudoterminal): CharacterDeviceDriver {
	...
	const deviceDriver: Pick<CharacterDeviceDriver, 'kind' | 'id' | 'uri' | 'createStdioFileDescriptor' | 'fd_fdstat_get' | 'fd_filestat_get' | 'fd_read' | 'fd_write' | 'fd_bytesAvailable'> = {
		...
		fd_bytesAvailable(_fileDescriptor: FileDescriptor): Promise<filesize> {
			return Promise.resolve(BigInts.max(0n, BigInt(terminal.bytesAvailable())));
		}
	 };
