...
import {
    ...
    Errno, Event, Event_fd_readwrite, event, Eventtype, exitcode, fd, fdflags, Fdstat, fdstat, fd_advise, fd_allocate, fd_close, fd_datasync, fd_fdstat_get, fd_fdstat_set_flags,
    ...
    subscription, thread_spawn, timestamp, WasiError, Whence, whence, thread_exit, tid, Preopentype, sock_shutdown, sock_send, sock_recv, roflags
} from './wasi';
...
export namespace EnvironmentWasiService {
	export function create(fileDescriptors: FileDescriptors, programName: string, preStats: IterableIterator<[string, DeviceDriver]>, options: EnvironmentOptions): EnvironmentWasiService {
		...
		const result: EnvironmentWasiService = {
			...
			fd_prestat_get: async (memory: ArrayBuffer, fd: fd, bufPtr: ptr<prestat>): Promise<errno> => {
				try {
					...
					prestat.len = $encoder.encode(dirname).byteLength;
			...
			fd_prestat_dir_name: (memory: ArrayBuffer, fd: fd, pathPtr: ptr<byte[]>, pathLen: size): Promise<errno> => {
				try {
					...
					return Promise.resolve(handleErrorPromise(error));
				}
			}
		};
		...
		function handleErrorPromise(error: any, def: errno = Errno.badf): Promise<errno> {
			return Promise.resolve(handleError(error, def));
		}
...
export namespace DeviceWasiService {
	export function create(deviceDrivers: DeviceDrivers, fileDescriptors: FileDescriptors, clock: Clock, virtualRootFileSystem: RootFileSystemDeviceDriver | undefined, options: DeviceOptions): DeviceWasiService {
		...
		const net: any = options?.net;
		let connFd: number = 0;
		let connFdUsed: boolean = false;
		...
		const result: DeviceWasiService = {
			...
			fd_fdstat_get: async (memory: ArrayBuffer, fd: fd, fdstat_ptr: ptr<fdstat>): Promise<errno> => {
				if ((connFd > 0) && (fd == connFd) && connFdUsed){
					let buffer = new DataView(memory);
			...
			fd_filestat_get: async (memory: ArrayBuffer, fd: fd, filestat_ptr: ptr<filestat>): Promise<errno> => {
				try {
					...
					return handleErrorPromise(error, Errno.perm);
				}
			},
			...
			fd_read: async (memory: ArrayBuffer, fd: fd, iovs_ptr: ptr<iovec>, iovs_len: u32, bytesRead_ptr: ptr<u32>): Promise<errno> => {
                                // if ((connFd > 0) && (fd == connFd)) {
                                //     return DeviceWasiService.sock_recv(memory, fd, iovs_ptr, iovs_len, 0, bytesRead_ptr, 0);
                                // }
				try {
					const fileDescriptor = getFileDescriptor(fd);
			...
			fd_write: async (memory: ArrayBuffer, fd: fd, ciovs_ptr: ptr<ciovec>, ciovs_len: u32, bytesWritten_ptr: ptr<u32>): Promise<errno> => {
                                // if ((connFd > 0) && (fd == connFd)) {
                                //     return sock_send(memory, fd, ciovs_ptr, ciovs_len, 0, bytesWritten_ptr);
                                // }
				try {
					const fileDescriptor = getFileDescriptor(fd);
			...
			poll_oneoff: async (memory: ArrayBuffer, input: ptr<subscription>, output: ptr<event[]>, subscriptions: size, result_size_ptr: ptr<u32>): Promise<errno> => {
				try {
					...
					for (const item of events) {
						let event = Event.create(view, event_offset);
			...
			sock_accept: (memory: ArrayBuffer, fd: fd, _flags: fdflags, result_fd_ptr: ptr<u32>): Promise<errno> => {
                            if (net == null) {
                                return Promise.resolve(Errno.nosys);
                            }
                            connFd = fileDescriptors.next();
                            if (connFdUsed) {
                                return Promise.resolve(Errno.inval);
                            }
                            try {
			    ...
			    connFdUsed = true;
                            const view = new DataView(memory);
                            view.setUint32(result_fd_ptr, connFd, true);
                            return Promise.resolve(Errno.success);
                            
			},
			sock_recv: (memory: ArrayBuffer, _fd: fd, ri_data_ptr: ptr<iovec>, ri_data_len: u32, _ri_flags: riflags, ro_datalen_ptr: ptr<u32>, _roflags_ptr: ptr<roflags>): Promise<errno> => {
                            if (net == null) {
                                return Promise.resolve(Errno.nosys);
                            }
                            // TODO: check fd
                            if (net.recv_is_readable() <= 0) {
                                return Promise.resolve(Errno.again);
                            }
			    try {
				const view = new DataView(memory);
				...
				view.setUint32(ro_datalen_ptr, bytesRead, true);
				return Promise.resolve(Errno.success);
			    } catch (error) {
				return handleErrorPromise(error);
			    }
			...
			sock_send: (memory: ArrayBuffer, _fd: fd, si_data_ptr: ptr<ciovec>, si_data_len: u32, _si_flags: siflags, si_datalen_ptr: ptr<u32>): Promise<errno> => {
                            if (net == null) {
                                return Promise.resolve(Errno.nosys);
                            }
                            // TODO: check fd
			...
			sock_shutdown: (_memory: ArrayBuffer, fd: fd, _sdflags: sdflags): Promise<errno> => {
			    // return Promise.resolve(Errno.nosys);
                            if ((connFd > 0) && (fd == connFd)) {
                                connFdUsed = false;
                            }
				...
				switch (u.type) {
					...
					case Eventtype.fd_read:
						const readEvent = await handleReadSubscription(subscription);
                                                if (readEvent.fd_readwrite.nbytes > 0) {
		...
		async function handleReadSubscription(subscription: subscription): Promise<Literal<event>> {
		    const fd = subscription.u.fd_read.file_descriptor;
                    if ((net != null) && (connFd > 0) && (fd === connFd)) {
                        const len: number = net.recv_is_readable();
                        return {
			    userdata: subscription.userdata,
...
export const NoSysWasiService: WasiService = {
	...
	sock_recv: (): Promise<number> => {
		throw new WasiError(Errno.nosys);
	},
	sock_send: (): Promise<number> => {
		throw new WasiError(Errno.nosys);
	},
