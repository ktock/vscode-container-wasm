...
import { advise, fd, fdflags, fdstat, filedelta, filesize, filestat, filetype, fstflags, lookupflags, oflags, rights, timestamp, whence } from './wasi';
...
export declare enum DeviceDriverKind {
    character = "character",
...
export declare namespace FileSystemDeviceDriver {
    function is(value: DeviceDriver): value is FileSystemDeviceDriver;
}
...
export interface ReadonlyFileSystemDeviceDriver extends Pick<FileSystemDeviceDriver, 'kind' | 'joinPath' | 'createStdioFileDescriptor' | 'uri' | 'id' | 'fd_advise' | 'fd_close' | 'fd_fdstat_get' | 'fd_filestat_get' | 'fd_pread' | 'fd_read' | 'fd_readdir' | 'fd_seek' | 'fd_renumber' | 'fd_tell' | 'path_filestat_get' | 'path_open' | 'path_readlink' | 'fd_create_prestat_fd' | 'fd_bytesAvailable'> {
    kind: DeviceDriverKind.fileSystem;
}
export declare const NoSysDeviceDriver: Omit<DeviceDriver, 'id' | 'uri' | 'kind'>;
export declare const WritePermDeniedDeviceDriver: Pick<DeviceDriver, 'fd_allocate' | 'fd_datasync' | 'fd_fdstat_set_flags' | 'fd_filestat_set_size' | 'fd_filestat_set_times' | 'fd_pwrite' | 'fd_renumber' | 'fd_sync' | 'fd_write' | 'path_create_directory' | 'path_filestat_set_times' | 'path_link' | 'path_remove_directory' | 'path_rename' | 'path_symlink' | 'path_unlink_file'>;
